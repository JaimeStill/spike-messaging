...
- **Modules:** a committed `go.work` layers the modules, as spike-harness-driver does. The root
  ...
  promoting a package is a move and the import graph tests the split. `courier` is the application's
  own module. A module's `go.mod` has no `require` for a workspace sibling, since `go.work` resolves
